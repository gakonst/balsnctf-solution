...
contract ERC223 {
    ...
    function helper(string memory customFallback, address who, uint256 value, bytes memory data) external view returns(bytes memory) {
        return abi.encodeWithSignature(customFallback, who, value, data);
    }
...
contract Election is ERC223 {
    ...
    function checkBallots(Ballot[] memory ballots) external pure returns (bytes32) {
    ...
    function helper2(address who, Proposal memory proposal) external view returns(bytes memory) {
        return abi.encodeWithSelector(this.propose.selector, who, proposal);
    }
}
